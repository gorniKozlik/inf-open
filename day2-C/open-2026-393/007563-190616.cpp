...
void solve() {
    ...
    ve<seg> left(all(segs));
    sort(all(left));
    ve<seg> right(all(segs));
    sort(all(right), cmp);
    ...
    while (i < n) {
        ...
        while (j < n && (used[right[j].idx] || right[j].l <= left[i].r)) {
            used[right[j].idx] = true;
            j++;
        }
        if (j >= n)
            break;
        if (left[i].idx == right[j].idx) {
            used[left[i].idx] = true;
            i++;
            continue;
        }
        int li = get(left[i].r), ri = get(right[j].l);
        tree.add(li, ri + 1, 1);
        tree.add(li + 1, ri, 1);
        mp.pb(tree.get());
        used[left[i].idx] = true;
