...
void Solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        one_cnt[count[vec[i].first] + cnt[vec[i].first] * 2] = std::max(one_cnt[count[vec[i].first] + cnt[vec[i].first] * 2], count[vec[i].first] + cnt[vec[i].first]);
        two_cnt[count[vec[i].first] + cnt[vec[i].first] * 2] = std::max(two_cnt[count[vec[i].first] + cnt[vec[i].first] * 2], count[vec[i].first]);
    }

    SegmentTree tree_two_cnt(two_cnt);
    SegmentTree tree_one_cnt(one_cnt);
