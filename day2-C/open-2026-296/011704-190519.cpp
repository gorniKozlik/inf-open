...
signed main() {
    ...
    for (int i = 0;i < n;i++) {
        ...
        useless.push_back({vec[i].first,1});
        useless.push_back({vec[i].second,-1});
    ...
    vector <pair<int,int>> can_used;
    for (auto el : useless) {
        if (el.second == -1) {
            if (max_cnt < cnt_now) {
                vert = el.first;
            }
            max_cnt = max(max_cnt,cnt_now);
            cnt_now--;
        }else {
            cnt_now++;
            if (max_cnt < cnt_now) {
            ...
            can_used.push_back({cnt_now,el.first});
            max_cnt = max(max_cnt,cnt_now);
        }
    }
    sort(can_used.rbegin(),can_used.rend());
    for (auto el : can_used) {
        if (1.0 * clock() / CLOCKS_PER_SEC >= 0.9)break;
        upd(vec,ans,el.second);
    }
    for (int i = n - 2;i >= 0;i--) {
