...
class sgt_r {
    ...
    ll get(int a, ll l, ll r, ll x) {
        if (l > x || r < x) return 0;
        if (l == r && l == x) return max(sg[a], d[a]);
        ll m = (l+r)/2;
        push(a);
        update(a);
        return max(get(a*2+1, l, m, x), get(a*2+2, m+1, r, x));
    }
...
int main() {
    ...
    for (int i = 0; i < 2*n; ++i) {
        ...
        dp.change(0, min(sch+min(sch1, n-sch-sch1)*2, n), sch);
    }   
    while (q--) {
        ll k;
        cin >> k;
        if (k <= dp.get_ans(k)) cout << 0 << ' ';
        else cout << (k-dp.get_ans(k)+1)/2 << ' ';
    }
}