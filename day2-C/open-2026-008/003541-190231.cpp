...
int main()
{
    ...
    for (auto& i : flow)
    {
        req.emplace_back(i.first, -1);
        req.emplace_back(i.second, 1);
    }
    for (size_t i = 1; i < m; i++)
    {
        req.emplace_back(i, 0);
    }
    ...
    for (size_t i = 0; i < q; i++)
    {
        ...
        int ov = lower_bound(tmp.begin(), tmp.end(), make_pair(ki, 0))->second;
        int ng = ki - ov;
        ans = (ng + 1) / 2;
        cout << ans << '\n';
    }
