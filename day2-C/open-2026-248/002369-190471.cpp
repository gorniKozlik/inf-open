...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cs.push_back(p[i].first);
        cs.push_back(p[i].second);
    }

    sort(cs.begin(), cs.end());

    for (auto &i : p) {
        i.first = lower_bound(cs.begin(), cs.end(), i.first) - cs.begin();
        i.second = lower_bound(cs.begin(), cs.end(), i.second) - cs.begin();

        add(1, 0, maxn, i.first, i.second, 1);
    }
