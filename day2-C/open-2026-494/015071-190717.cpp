...
signed main() {
	...
	if (q == 1) {
		...
		for (int i = 2 * n - 1; i >= 0; --i) {
			...
			int l = vsp[i].l;
			int r = vsp[i].r;
			if (bal + min(l / 2, r / 2) * 2 + l / 2 - min(l / 2, r / 2) >= x) {
				int op = 0;
				if (bal + 2 * min(r / 2, l / 2) >= x) {
					op = (x - bal + 1) / 2;
				}
				else {
					op = min(r / 2, l / 2) + (x - bal - 2 * min(r / 2, l / 2));
				}
				//cout << i << ' ' << t[i].first << ' ' << op << ' ' << l << ' ' << r << ' ' << bal << endl;
				mn = min(mn, op);
			}
		}
		cout << mn << endl;
	...
	set<pair<int, int>> s;
	for (int i = 0; i < 2 * n; ++i) {
		s.insert({ vsp[i].bal + 2 * min(vsp[i].l / 2, vsp[i].r / 2), i });
	}
	/*for (int w = 0; w < 2 * n; ++w) {
		int i = s.begin()->second;
		s.erase(s.begin());
	...
	for (int i = 0; i < q; ++i) {
		...
		for (auto j : s) {
			...
			int l = vsp[y].l;
			int r = vsp[y].r;
			if (bal + min(l / 2, r / 2) * 2 + l / 2 - min(l / 2, r / 2) >= k[i]) {
				int op = 0;
				if (bal + 2 * min(r / 2, l / 2) >= k[i]) {
					op = (k[i] - bal + 1) / 2;
				}
				else {
					op = min(r / 2, l / 2) + (k[i] - bal - 2 * min(r / 2, l / 2));
				}
				//cout << i << ' ' << t[i].first << ' ' << op << ' ' << l << ' ' << r << ' ' << bal << endl;
				res = min(res, op);
			}
		}
		for (int y = max(0LL, n - 50); y <= min(2 * n - 1, n + 25); ++y) {
			int bal = vsp[y].bal;
			int l = vsp[y].l;
			int r = vsp[y].r;
			if (bal + min(l / 2, r / 2) * 2 + l / 2 - min(l / 2, r / 2) >= k[i]) {
