...
void solve() {
    ...
    vector<Covered> best_covered_4 = best_covered_1;
    reverse(best_covered_4.begin(), best_covered_4.end());
    vector<Covered> best_covered_5 = best_covered_2;
    reverse(best_covered_5.begin(), best_covered_5.end());
    vector<Covered> best_covered_6 = best_covered_3;
    reverse(best_covered_6.begin(), best_covered_6.end());
    best_covered_1.resize(min(sz(best_covered), 100));
    best_covered_2.resize(min(sz(best_covered), 100));
    best_covered_3.resize(min(sz(best_covered), 100));
    best_covered_4.resize(min(sz(best_covered), 100));
    best_covered_5.resize(min(sz(best_covered), 100));
    best_covered_6.resize(min(sz(best_covered), 100));
    while (q--) {
        int k;
        cin >> k;
        int ans = INF;
        ...
        for (auto [a, b] : best_covered_5) {
            ans = min(ans, min(max(0, (k - a + 1) / 2), b) + max(0, k - a - 2 * b));
        }
        for (auto [a, b] : best_covered_6) {
            ans = min(ans, min(max(0, (k - a + 1) / 2), b) + max(0, k - a - 2 * b));
        }
        cout << ans << '\n';
    }
