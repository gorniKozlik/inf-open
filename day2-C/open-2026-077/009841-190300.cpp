...
signed main() {
    ...
    // cout << "FF " << f(n, n) << endl;
    // cout << "n " << n << ' ' << lx.size() << endl;
    // return 0;

    for (int b = 1; b <= n; ++b) {
        while (it < (int)srt.size() - 1) {
            if (f(b, srt[it].second) > f(b, srt[it + 1].second)) ++it;
            else break;
        }

        // for (int i = max((int)0, it - 100); i < min((int)stk.size(), it + 100); ++i) {
        //     ans[b] = min(ans[b], f(b, stk[i].second));
        // }
        for (int i = 0; i < srt.size(); ++i) {
            ans[b] = min(ans[b], f(b, srt[i].second));
        }
        // for (int i = 0; i < lx.size(); ++i) {
        //     ans[b] = min(ans[b], f(b, i));
        // }
