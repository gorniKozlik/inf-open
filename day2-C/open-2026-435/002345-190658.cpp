...
struct SegmentTree {
    ...
    void build(int pos, int left, int right, const vector<int>& pref) {
        if (right - left == 1) {
            tree[pos].add = 0;
    ...
    void build(const vector<int>& pref) {
        build(0, 0, n, pref);
    }

    void add(int lb, int rb, int val, int pos, int left, int right) {
        push(pos, left, right);
    ...
    int RMQ(int lb, int rb, int pos, int left, int right) {
        push(pos, left, right);
        if (left >= rb || lb >= right) return -INF;
        if (lb <= left && right <= rb) return tree[pos].max;
        else {
            int mid = (left + right) / 2;
            int lc = RMQ(lb, rb, 2 * pos + 1, left, mid);
            int rc = RMQ(lb, rb, 2 * pos + 2, mid, right);
            return max(lc, rc);
        }
    }

    int RMQ(int lb, int rb) {
        return RMQ(lb, rb, 0, 0, n);
    }
};
...
int main() {
    ...
    for (int i = 0; i < 2 * n; ++i) {
        ...
        if (arr[i].type == LEFT) lx.push_back({arr[i].x, i});
        if (arr[i].type == RIGHT) rx.push_back({arr[i].x, i});
    }
    ...
    while (true) {
        ...
        while (ptr_not_ans < nQ && qs[ptr_not_ans].k <= mx_c) {
            ans[qs[ptr_not_ans].ind] = cur_ops;
            ++ptr_not_ans;
        }
        if (ptr_not_ans == nQ) break;

        pair<int, int> lmr = rx[ptr_rx++];
        pair<int, int> rml = lx[ptr_lx--];
