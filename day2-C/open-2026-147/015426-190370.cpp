...
int32_t main() {
    ...
    for (int i = b.size() - 1; i + 1; i--) mx1[i] = max((i - b.size() + 1 ? mx1[i + 1] : (int)-1e18), vl[r[i].y]);
    for (int i = 0; i <= n; i++) {
        while (lst - b.size() + 1 && r[lst + 1].x < i) lst++, mx = max(mx, v1[r[lst].y]);
        int cmx = max(mx + i, (lst - b.size() + 1 ? mx1[lst + 1] : (int)-1e18) + (i << 1));
        rs.push_back(cmx);
    }
    for (auto &i : qr) {
