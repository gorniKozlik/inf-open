...
void solve(){
    ...
    while (cnt < mx){
        ...
        p = *st.begin(), p1 = *(--st1.end());
        int k = min(mp[*st.begin()], mp1[*(--st1.end())]);
        update(1, 0, 2 * n, p.se, p.fr, -k);
        update(1, 0, 2 * n, p1.fr, p1.se, -k);
        update(1, 0, 2 * n, p.se, p1.se, k);
        update(1, 0, 2 * n, p.fr, p1.fr, k);
        mp[*st.begin()] -= k, mp1[*(--st1.end())] -= k;
        if (mp[*st.begin()] == 0){
            st.erase(st.begin());
        }
        if (mp1[*(--st1.end())] == 0){
