...
int main() {
    ...
    vector<vector<long long>> starts(n + 10), ends(n + 10);
    for (auto &j : all) {
        ...
        // }
        pref = max(pref, alr);
        starts[alr].push_back(alr);
        ends[2 * min(cnt_l, cnt_r) + alr + 1].push_back(alr);
    ...
    for (long long i = 0; i < answers.size(); i++) {
        ...
        if (!now.empty()) {
            answers[i] = min(answers[i], (i - *now.rbegin() + 1) / 2);
        }
    }
    while (q--) {
        long long k;
