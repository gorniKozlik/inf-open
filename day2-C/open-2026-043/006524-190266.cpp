...
int main() {
    ...
    while (q--) {
        ...
        long long ans = 1e18;
        for (auto &j : all) {
            long long cnt_l = 0, cnt_r = 0, alr = 0;
            for (auto &[l, r] : a) {
                if (r < j) {
                    cnt_l++;
                } else if (j < l) {
            ...
            if (alr >= k) {
                ans = min(ans, 0ll);
            } else if (2 * min(cnt_l, cnt_r) + alr >= k) {
                ans = min(ans, (k - alr + 1) / 2);
            } else if (k == 2 * min(cnt_l, cnt_r) + alr + 1 && cnt_l != cnt_r) {
                ans = min(ans, (k - alr + 1 - 1) / 2 + 1);
            }
        }
        cout << ans << endl;
    }
