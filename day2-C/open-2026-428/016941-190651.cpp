...
int main() {
	...
	prefstarts.reserve(2 * n + 1);
	prefends.reserve(2 * n + 1);
	ll starts = 0, ends = 0;
	...
	unordered_map<ll, bool> canstart;
	for (ll i = 0; i < 2 * n; i++) {
		ll leftcnt = prefends[i],
			rightcnt = prefstarts[2 * n] - prefstarts[i + 1];
		ll nowcnt = n - leftcnt - rightcnt;
		minqueries[nowcnt] = 0;
		ll add = min(leftcnt, rightcnt) * 2;
		canstart[nowcnt + add] = true;
		minqueries[nowcnt + add - 1] = min(minqueries[nowcnt + add - 1], add / 2);
	...
	ll nowmin = INT32_MAX, collecter = 0;
	for (ll i = n; i > -1; i--) {
		nowmin -= collecter / 2;
		collecter %= 2;
		if (nowmin < 0) {
		...
		else minqueries[i] = nowmin;
		collecter++;
	}
	while (q--) {
		ll k;
		cin >> k;
		assert(minqueries[k] != INT32_MAX);
		cout << minqueries[k] << " ";
	}
}
