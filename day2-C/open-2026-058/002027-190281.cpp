...
void solve() {
    ...
    for (int c: cord) {
        int LEFT = upper_bound(r.begin(), r.end(), c) - r.begin(); // меньших
        int RIGHT = n - (lower_bound(l.begin(), l.end(), c) - l.begin()); // больших
        int tyt = col[ind];
        int lish = (LEFT + RIGHT + tyt - n);
        for (int i = 0; i <= min(LEFT, RIGHT); ++i, tyt += 2) {
            if (i > min(LEFT, RIGHT) - lish) {
                tyt--;
            }
