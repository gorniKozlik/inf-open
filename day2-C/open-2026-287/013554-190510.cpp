...
void solve() {
    ...
    auto get_max = [&] {
        ...
        return make_pair(maxcnt, max_x);
    };

    vector <int> ans(n + 1, INF);
    auto setans = [&](int ops) {
        int x = get_max().f + above;
        for (int i = 0; i <= x; i++) {
            ans[i] = min(ops, ans[i]);
        }
    };
    ...
    while (!a.empty()) {
        ...
        x = get_max().s;
        // ищем отрезок с r < x и с l > x, переподвесим друг за друга
        // если нет, то сделаем как обычно хз
