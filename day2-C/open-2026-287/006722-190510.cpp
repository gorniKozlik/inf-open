...
void solve() {
    ...
    auto get_above = [&]() {
        ...
        for (int i = 0; i < a.size(); i++) max_l = max(max_l, a[i].l), min_r = min(min_r, a[i].r);
        for (int i = 0; i < a.size(); i++) {
            if (a[i].l < min_r && a[i].r > max_l) return i;
        }
        return -1;
    };
    ...
    while (!a.empty()) {
        ...
        if (min_l == max_r) { // какого фига, ну ладно
            a.erase(a.begin() + min_l);
            above++;
            continue;
        }
