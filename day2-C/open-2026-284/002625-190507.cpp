...
signed main()
{
	...
	for (int i = 0; i < min((long long)d.size() * 1ll, 1000ll); i++)
	{
		int c = d[i].first;
		int b = d[i].second;
	...
	for (int i = 0; i < d.size(); i++)
	{
		q.push_back({ d[i].first + d[i].second, d[i].first, d[i].second });
	}
	...
	for (int i = 0; i < min((long long)q.size(), 1000ll); i++)
	{
		int c = q[i][2];
		int b = q[i][1];
		vector<int> ax(n + 1);
