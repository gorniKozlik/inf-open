...
void solve() {
    ...
    for (auto [l, t]: events) {
        ...
        int mx = cnt_open + min(cnt_closed, n - cnt_closed - cnt_open) * 2 + (
                     1 - (cnt_closed == n - cnt_closed - cnt_open));
        int cost = min(cnt_closed, n - cnt_closed - cnt_open) + (1 - (cnt_closed == n - cnt_closed - cnt_open));
        if ((1 - (cnt_closed == n - cnt_closed - cnt_open))) {
            an[mx - 1] = min(an[mx - 1], cost - 1);
        }
