...
void solve() {
  ...
  for (ll i = 0; i < 2 * n; ++i) {
    while (st.size() && (*st.begin())[0] == i && (*st.begin())[1] == 0) {
      ...
      if (n - cur - cnt < cnt) {
        kost.insert({ cur + 1, 0ll, 2 * n + i });
        ind[2 * n + i] = -cur + 2;
        kost.insert({ cur + 1 + 2 * min(cnt - 1, n - cur - cnt), 1ll, 2 * n + i });
      }
    }
    ...
    while (st.size() && (*st.begin())[0] == i && (*st.begin())[1] == 1) {
      ...
      if (n - cur - cnt > cnt) {
        kost.insert({ cur + 1, 0ll, n + i });
        ind[n + i] = -cur + 2;
        kost.insert({ cur + 1 + 2 * min(cnt, n - cur - cnt - 1), 1ll, n + i });
      }
  ...
  for (ll i = 1; i <= n; ++i) {
    ...
    while (kost.size() && (*kost.begin())[0] == i && (*kost.begin())[1] == 0) {
      auto t = *kost.begin();
      kost.erase(t);

      tmp.insert({ -t[0] + 2, t[2] });
    }
    ...
    while (kost.size() && (*kost.begin())[0] == i && (*kost.begin())[1] == 1) {
      auto t = *kost.begin();
      kost.erase(t);
