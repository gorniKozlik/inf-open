...
void solve() {
  ...
  for (ll i = 0; i < 2 * n; ++i) {
    if (st.size() && (*st.begin())[0] == i && (*st.begin())[1] == 0) {
      auto t = *st.begin();
      st.erase(t);
      ++cur;
    }
    ...
    mx = max(mx, cur);
    ot.emplace(pii<3>{ cur, 0ll, i });
    ind[i] = -cur;
    ot.emplace(pii<3>{ cur + 2 * min(cnt, n - cur - cnt), 1ll, i });

    if (st.size() && (*st.begin())[0] == i && (*st.begin())[1] == 1) {
      st.erase(st.begin());
      ++cnt;
      --cur;
    }

    ot.emplace(pii<3>{ cur, 0ll, 2 * n + i });
    ind[2 * n + i] = -cur;
    ot.emplace(pii<3>{ cur + 2 * min(cnt, n - cur - cnt), 1ll, 2 * n + i });
  }
  st.clear();
