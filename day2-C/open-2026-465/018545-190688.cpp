...
signed main() {
    ...
    for (int i = 0; i < 2 * n; ++i) {
        if (suf_l[i] + pref_r[i] > max_cnt) {
            max_cnt = suf_l[i] + pref_r[i];
            poses.push_back({max_cnt, i});
        }
    }
    sort(poses.begin(), poses.end());
    reverse(poses.begin(), poses.end());
    set<int> lol;
    for (int i = 0; i < min((int)400, (int)poses.size()); ++i) {
        lol.insert(poses[i].second);
    }
    ...
    for (int i = 0; i < n * 2 - 1; ++i) {
        ...
        if (lol.find(i)==lol.end()) { continue; }
        int lb = -1, rb = i, mid;
        while (rb - lb > 1) {
            mid = (lb + rb) / 2;
