...
signed main() {
    ...
    for (int i = 0; i < n * 2 - 1; ++i) {
        ...
        int val = min(cnt, p1 - min(p1, p2));
        int can = cnt + min(p1, p2) * 2 + val + min(cnt - val, p2 - min(p1, p2));
        ans[can] = min(ans[can], min(p1, p2) + val + min(cnt - val, p2 - min(p1, p2)));
    }
    int val = inf;
    for (int j = n; j >= 0; --j) {
        val = min(max((int) 0, val - 1), ans[j]);
        ans[j] = val;
    }
    while (q--) {
        int val;
