...
void solve() {
    ...
    vector<vector<ll>> open(n + 1), close(n + 1);
    ...
    for (auto& [x, y] : cnt) {
        ...
        zeros[balance] = 0;
        if (balance + 1 <= maxi) {
            open[balance + 1].push_back(1);
            close[maxi].push_back(balance + 1);
        }
    ...
    multiset<ll> ids;
    for (ll i = 0; i <= n; ++i) {
        for (ll j = 0; j < len(open[i]); ++j) {
            ids.insert(i);
        }
        if (len(ids) > 0) {
            ans[i] = min(ans[i], (i - *ids.rbegin()) / 2 + 1);
        }
        for (ll j = 0; j < len(close[i]); ++j) {
            ids.erase(ids.lower_bound(close[i][j]));
        }
    }
