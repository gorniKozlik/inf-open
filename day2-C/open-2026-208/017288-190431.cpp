...
signed main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cnt_used[a[i].first * len * 2 + a[i].second]++;
    ...
    for (; left < n && right >= 0;) {
        ...
        while (left < n && cnt_used[min_left[left].first * len * 2 + min_left[left].second] == used[min_left[left].first * len * 2 + min_left[left].second]) {
            left++;
        }
        if (left < n) {
            used[min_left[left].first * len * 2 + min_left[left].second]++;
        }
        while (right >= 0 && cnt_used[max_right[right].first * len * 2 + max_right[right].second] == used[max_right[right].first * len * 2 + max_right[right].second] || left == right) {
            right--;
        }
        if (right >= 0) {
            used[max_right[right].first * len * 2 + max_right[right].second]++;
        }
        //cout << left << ' ' << right << endl;
        if (left < n && right >= 0) {
