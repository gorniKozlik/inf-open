...
signed main() {
    ...
    ll len = (int)coord.size();
    //vector<int> cnt_used(len * 4), used(len * 4);
    gp_hash_table<ll, int> cnt_used, used;
    for (int i = 0; i < n; i++) {
    ...
    for (; left < n && right >= 0;) {
        ...
        while (right >= 0 && cnt_used[max_right[right].first * len + max_right[right].second] == used[max_right[right].first * len + max_right[right].second]) {
            right--;
        }
        if (right >= 0) {
