...
void build(ll m, ll l, ll r, ll now) {
    if (r - l == 1) {
        ...
        tree[m].max_tree = max(0ll, now + tree[m].a);
        return;
    }
    ...
    tree[m].max_tree = max(0ll, tree[m].max_tree);
}

void update(ll m, ll l, ll r, ll ql, ll qr) {
    if (r <= ql || qr <= l) {
        return;
    }
    if (r - l == 1) {
    ...
    tree[m * 2 + 1].max_tree += tree[m].add;
    tree[m * 2 + 1].add += tree[m].add;
    tree[m * 2 + 2].max_tree += tree[m].add;
    tree[m * 2 + 2].add += tree[m].add;
    tree[m].add = 0;

    update(m * 2 + 1, l, (l + r) / 2, ql, qr);
    update(m * 2 + 2, (l + r) / 2, r, ql, qr);

    tree[m].max_tree = max(tree[m * 2 + 1].max_tree, tree[m * 2 + 2].max_tree);
}
...
int main() {
    ...
    while (l < r) {
        ...
        if (l < r) {
            update(0, 0, n * 2, inds[a[l].second], inds[a[r].first]);
