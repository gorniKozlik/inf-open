...
void solve() {
    ...
    vals.resize(ul - vals.begin());
    for (int i = 0; i <n ; i++) {
        a[i].first = lower_bound(all(vals), a[i].first) - vals.begin() + 1;
        a[i].second = lower_bound(all(vals), a[i].second) - vals.begin() + 1;
    }
    for (int i = 0; i < n; i++) {
