...
void solve() {
    ...
    sort(all(vals));
    auto ul = unique(all(vals));
    for (int i = 0; i < n; i++) {
        a[i].first = lower_bound(vals.begin(), ul, a[i].first) - vals.begin() + 1;
        a[i].second = lower_bound(vals.begin(), ul, a[i].second) - vals.begin() + 1;
    }
    ...
    int cnt = 0;
    for (int i = 0; i < q; i++) {
        while (cnt < n && get(0, mxn) < qw[i].first) {
            if (cnt > n) exit(0);
            int mnf = f.begin()->second;
            int mxs = st.begin()->second;
            upd(a[mnf].second + 1, a[mxs].first, 1);
            upd(a[mnf].second, a[mxs].first + 1, 1);
