...
*/
	...
	for (int i = 0; i < 2 * n; i++) {
		...
		count2 += v[i].second;
		if (count2 == mcount) {
			ct.push_back(abs(count3 - count2));
			mct.push_back(abs(n - count3));
			

		}
	}
	for (int i = 0; i < size(ct); i++) {
		ct1.push_back(abs(mct[i] - ct[i]));
	}
	ll j2 = MOD, j1 = 0;
	...
	ll cnt3 = abs(ct[j1]);
	ll cnt2 = n - abs(cnt3) - mcount;
	ll p = min(abs(cnt3), abs(cnt2));
	vector<ll> k(n + 1, 0);
